...
#define ZSHA (memcpy(&ctx_sha, &z_sha, sizeof(z_sha)))

/* ----------- Sudo Hash ------------------------------------------------- */
/** A hasher class for Sudo's 256-bit hash (double SHA-256). */
class CHash256 {
private:
    CSHA256 sha;
public:
...
/** A hasher class for Sudo's 160-bit hash (SHA-256 + RIPEMD-160). */
class CHash160 {
private:
    CSHA256 sha;
public:
