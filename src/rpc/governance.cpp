...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in sudo.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("sudo.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Sudo features */
    { "sudo",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "sudo",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "sudo",               "gobject",                &gobject,                true,  {} },
    { "sudo",               "voteraw",                &voteraw,                true,  {} },
