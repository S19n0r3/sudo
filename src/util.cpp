...
const char * const BITCOIN_CONF_FILENAME = "sudo.conf";
const char * const BITCOIN_PID_FILENAME = "sudod.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "sudo" is a composite category enabling all Sudo-related debug output
                if(ptrCategory->count(std::string("sudo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SudoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SudoCore
    // Mac: ~/Library/Application Support/SudoCore
    // Unix: ~/.sudocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SudoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SudoCore";
#else
    // Unix
    return pathRet / ".sudocore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty sudo.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override sudo.conf
